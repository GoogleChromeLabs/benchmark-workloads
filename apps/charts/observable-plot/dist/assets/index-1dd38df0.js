...
function ticker(year, month, week, day, hour, minute) {
  ...
  function tickInterval(start2, stop, count) {
    ...
    if (i === tickIntervals2.length)
      return year.every(tickStep(start2 / durationYear$1, stop / durationYear$1, count));
    if (i === 0)
...
function hasX({ x, x1: x12, x2 }) {
  return x !== void 0 || x12 !== void 0 || x2 !== void 0;
}
function hasY({ y, y1: y12, y2 }) {
  return y !== void 0 || y12 !== void 0 || y2 !== void 0;
}
...
function take(values2, index2) {
  return map(index2, (i) => values2[i], values2.constructor);
}
...
function composeTransform(t12, t22) {
  ...
  return function(data, facets, plotOptions) {
    ...
    return t22.call(this, arrayify(data), facets, plotOptions);
  };
}
...
function channelDomain(data, facets, channels, facetChannels, options) {
  ...
  for (const x in options) {
    if (!registry.has(x))
      continue;
    let { value: y, order = defaultOrder, reverse: reverse2 = defaultReverse, reduce = defaultReduce, limit = defaultLimit } = maybeValue(options[x]);
    const negate = y == null ? void 0 : y.startsWith("-");
    if (negate)
      y = y.slice(1);
    order = order === void 0 ? negate !== (y === "width" || y === "height") ? descendingGroup : ascendingGroup : maybeOrder$1(order);
...
function maybeClip(clip2) {
  if (clip2 === true)
    ...
    clip2 = keyword(clip2, "clip", ["frame", "sphere"]);
  return clip2;
}
function applyClip(selection2, mark, dimensions, context) {
  ...
  switch (clip2) {
    case "frame": {
      ...
      const id2 = getClipId();
      clipUrl = `url(#${id2})`;
      selection2 = create("svg:g", context).call(
...
function createScaleQ(key, scale, channels, {
  ...
  if (nice2)
    scale.nice(maybeNice(nice2, type)), domain = scale.domain();
  if (range2 !== void 0)
    scale.range(range2);
  if (clamp)
    scale.clamp(clamp);
...
function inferScaleType(key, channels, { type, domain, range: range2, scheme: scheme2, pivot, projection: projection2 }) {
  ...
  if (kind === color) {
    if (pivot != null || isDivergingScheme(scheme2))
      return "diverging";
    if (isCategoricalScheme(scheme2))
      return "categorical";
  }
  return "linear";
}
...
function coerceType(channels, { domain, ...options }, coerceValues) {
  ...
  for (const c2 of channels) {
    if (c2.value !== void 0) {
      if (domain === void 0)
        domain = (_a = c2.value) == null ? void 0 : _a.domain;
...
function pointerK(kx2, ky2, { x, y, px, py, maxRadius = 40, channels, render, ...options } = {}) {
  maxRadius = +maxRadius;
  if (px != null)
...
class RuleX extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X && x }, offset, 0).call(
      (g) => g.selectAll().data(index2).enter().append("line").call(applyDirectStyles, this).attr("x1", X ? (i) => X[i] : (marginLeft + width - marginRight) / 2).attr("x2", X ? (i) => X[i] : (marginLeft + width - marginRight) / 2).attr("y1", Y12 && !isCollapsed(y) ? (i) => Y12[i] + insetTop : marginTop + insetTop).attr(
        "y2",
        Y22 && !isCollapsed(y) ? y.bandwidth ? (i) => Y22[i] + y.bandwidth() - insetBottom : (i) => Y22[i] - insetBottom : height - marginBottom - insetBottom
...
class RuleY extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { y: Y && y }, 0, offset).call(
      (g) => g.selectAll().data(index2).enter().append("line").call(applyDirectStyles, this).attr("x1", X12 && !isCollapsed(x) ? (i) => X12[i] + insetLeft : marginLeft + insetLeft).attr(
        "x2",
        X22 && !isCollapsed(x) ? x.bandwidth ? (i) => X22[i] + x.bandwidth() - insetRight : (i) => X22[i] - insetRight : width - marginRight - insetRight
...
function gridKy(k2, anchor, data, {
  ...
  return axisMark(ruleY, k2, anchor, `${k2}-grid`, data, { y, x1: x12, x2, ...gridDefaults(options) });
}
function gridKx(k2, anchor, data, {
  ...
  return axisMark(ruleX, k2, anchor, `${k2}-grid`, data, { x, y1: y12, y2, ...gridDefaults(options) });
}
      ...
      if (isIterable(ticks2)) {
        ...
        if (scale.ticks) {
          ...
          if (interval3 !== scale.interval)
            data2 = inclusiveRange(interval3, ...extent$1(data2));
        }
        if (interval3 === scale.interval) {
...
class Frame extends Mark {
  constructor(options = {}) {
    ...
    super(singleton, void 0, options, anchor == null ? defaults$3 : lineDefaults);
...
class Tip extends Mark {
  ...
  render(index2, scales, values2, dimensions, context) {
    ...
    const g = create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyIndirectTextStyles, this).call(applyTransform, this, { x: X && x, y: Y && y }).call(
      (g2) => g2.selectAll().data(index2).enter().append("g").attr("transform", (i) => `translate(${Math.round(px(i))},${Math.round(py(i))})`).call(applyDirectStyles, this).call((g3) => g3.append("path").attr("filter", pathFilter)).call(
        (g3) => g3.append("text").each(function(i) {
          ...
          const lines = format2.call(mark, i, index2, sources, scales, values2);
    ...
    function renderLine(selection2, { label, value, color: color2, opacity: opacity2 }) {
      ...
      if (swatch)
        line.append("tspan").text(" ■").attr("fill", color2).attr("fill-opacity", opacity2).style("user-select", "none");
      if (title)
...
function getSourceChannels({ channels }, scales) {
  ...
  for (const key in channels) {
    if (key in sources || key in format2 || ignoreChannels.has(key))
      continue;
    const source = getSource(channels, key);
    if (source)
      sources[key] = source;
  ...
  if (this.facet) {
    if (scales.fx && !("fx" in format2))
      sources.fx = true;
    if (scales.fy && !("fy" in format2))
      sources.fy = true;
  }
...
function maybeExpandPairedFormat(format2, channels, key) {
  ...
  const key1 = `${key}1`;
  const key2 = `${key}2`;
  if ((key1 in format2 || !(key1 in channels)) && (key2 in format2 || !(key2 in channels)))
    return format2;
...
function plot(options = {}) {
  ...
  const svg = creator("svg").call(document2.documentElement);
  let figure = svg;
  ...
  select(svg).attr("class", className).attr("fill", "currentColor").attr("font-family", "system-ui, sans-serif").attr("font-size", 10).attr("text-anchor", "middle").attr("width", width).attr("height", height).attr("viewBox", `0 0 ${width} ${height}`).attr("aria-label", ariaLabel).attr("aria-description", ariaDescription).call(
    (svg2) => (
      // Warning: if you edit this, change defaultClassName.
      svg2.append("style").text(
        `:where(.${className}) {
    ...
    if (facets === void 0 || mark.facet === "super") {
      ...
      for (const f of facets) {
        ...
        if (indexes) {
          ...
          index2 = indexes[faceted ? f.i : 0];
          index2 = mark.filter(index2, channels, values2);
          if (index2.length === 0)
            continue;
          if (!faceted && index2 === indexes[0])
        ...
        if (node == null)
          continue;
        (g ?? (g = select(svg).append("g"))).append(() => node).datum(f);
...
function applyScaleTransform(channel, options) {
  ...
  const {
    ...
    percent,
    interval: interval2,
    transform = percent ? (x) => x * 100 : maybeIntervalTransform(interval2, type)
  } = options[scale] ?? {};
  if (transform == null)
    return;
...
function maybeMarkFacet(mark, topFacetState, options) {
  ...
  if (data.length > 0 && (groups2.size > 1 || groups2.size === 1 && channels.fx && channels.fy && [...groups2][0][1].size > 1) && ((_a = arrayify(mark.data)) == null ? void 0 : _a.length) === data.length) {
    warn(
...
function stackY(stackOptions = {}, options = {}) {
  ...
  const [transform, X, y12, y2] = stack(x, y, "x", "y", stackOptions, rest);
  return { ...transform, x1: x12, x: X, y1: y12, y2, y: mid(y12, y2) };
}
...
function stack(x, y = one, kx2, ky2, { offset: offset2, order, reverse: reverse2 }, options) {
  if (y === null)
  ...
  return [
    basic(options, (data, facets, plotOptions) => {
      ...
      const X3 = x == null ? void 0 : setX(maybeApplyInterval(valueof(data, x), plotOptions == null ? void 0 : plotOptions[kx2]));
...
function barY(data, options = {}) {
  if (!hasXY(options))
    options = { ...options, x: indexOf, y2: identity$1 };
...
class Dot extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    const [cx, cy] = applyFrameAnchor(this, dimensions);
    const circle = symbol2 === symbolCircle;
    const size = R ? void 0 : r * r * Math.PI;
    if (negative(r))
      index2 = [];
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X && x, y: Y && y }).call(
      (g) => g.selectAll().data(index2).enter().append(circle ? "circle" : "path").call(applyDirectStyles, this).call(
        circle ? (selection2) => {
...
window.name = "observable-plot";
window.version = "1.0.0";
function triggerEvent(t, e, n) {
  const a2 = new CustomEvent(e);
  a2.name = e, a2.state = n, t.dispatchEvent(a2);
}
...
window.onmessage = async (t) => {
  ...
  n && (requestAnimationFrame(() => {
    ...
    await n(), setTimeout(() => {
      ...
      const t4 = JSON.stringify(performance.getEntriesByName(`${e}`)[0]);
      window.top.postMessage({ type: "test-completed", status: "success", result: t4 }, "*");
    }, 0);
  }));
}, window.top.postMessage({ type: "app-ready", status: "success", appId }, "*"), console.log(`Hello, benchmark connector for ${appId} is ready!`);
//# sourceMappingURL=index-1dd38df0.js.map
